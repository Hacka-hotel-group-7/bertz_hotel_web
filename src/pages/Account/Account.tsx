    ...
    return (
      <>
        ...
        <main>
          ...
          <div>
            <h3>Olá, {user?.name}</h3>
            <ButtonStyled onClick={() => setUserSettings("editProfile")}>
              Editar conta
            </ButtonStyled>
            <div className="menu_buttons">
              <ButtonStyled onClick={() => setUserSettings("reservations")}>
                Suas Reservas
              </ButtonStyled>
              <ButtonStyled
                onClick={() => setUserSettings("createReservation")}
              >
                Faça uma reserva
              </ButtonStyled>
              <ButtonStyled onClick={() => setUserSettings("reviews")}>
                Seus Comentários
              </ButtonStyled>
              <ButtonStyled onClick={() => setUserSettings("createReviews")}>
                Faça um comentário
              </ButtonStyled>
            </div>
          </div>
          ...
          <div>
            <ul>
              ...
              user.reservations.length > 0 ? (
                ...
                userSettings === "reservations" &&
                user.reservations.length === 0 ? (
                <StyledH2 fontWeight="extrabold">
                  Você ainda não possui reservas
                </StyledH2>
              ) : null}
              ...
              {user && userSettings === "reviews" && user.reviews.length > 0 ? (
                ...
                userSettings === "reviews" &&
                user.reviews.length === 0 ? (
                <StyledH2 fontWeight="extrabold">
                  Você ainda não fez comentários
                </StyledH2>
              ) : null}
              ...
              {user && userSettings === "createReservation"
                ? BedroomsList.map((bedroom) => (
                    <li key={bedroom.id}>
                      <StyledP fontWeight="bold" fontSize="medium">
                        Quarto: {bedroom.room_type}
                      </StyledP>
                      <img src={bedroom.image} alt="Imagem do quarto" />
                      <ButtonStyled
                        onClick={() => navigate(`/booking/${bedroom.id}`)}
                      >
                        Faça sua reserva
                      </ButtonStyled>
                    </li>
